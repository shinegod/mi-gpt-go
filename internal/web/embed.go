...
// StaticFileSystem 包装embed.FS以提供http.FileSystem接口
type StaticFileSystem struct {
	http.FileSystem
}

// GetStaticFS 获取静态文件系统（完整的static目录）
...
// GetAssetsFS 获取assets文件系统（仅assets子目录）
func GetAssetsFS() http.FileSystem {
	sub, err := fs.Sub(staticFiles, "static/assets")
	if err != nil {
		panic(err)
	}
	return http.FS(sub)
